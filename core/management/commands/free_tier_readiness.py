...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        add('Carga de casos', ClinicalCase.objects.filter(active=True).count() == 80, f'ativos={ClinicalCase.objects.filter(active=True).count()}', 'P1')
        add('Superusuário único', User.objects.filter(is_superuser=True).count() == 1, f'total={User.objects.filter(is_superuser=True).count()}', 'P0')
